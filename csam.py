...
def message_send(message):
	...
	starter= f'''
	©️ Привет, {username}! Это бот для покупки валюты в разных играх.
...
def handler_call(call):
	...
	if call.data == 'buy':
		...
		helpp = '''
		🎓 Попал в бота, но не знаешь, что делать? Тогда ознакомься с нашим обучением:

		Это бот для покупки валюты для таких игр как: Pubg mobile, Standoff 2, Brawl Stars.
